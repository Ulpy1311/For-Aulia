...
import { ArrowLeft, BookOpen, Moon, Sun, Globe, Image, Cloud, List } from 'lucide-react';
...
export function CustomContextMenu() {
    ...
    const menuItems: { label: string; icon: React.ReactNode; action: (e?: any) => void }[] = [
        ...(pathname !== '/' ? [{
            label: 'Back to Home',
        ...
        },
        {
            ...
            icon: <List className="w-4 h-4" />, // Changed icon to List for Changelog
            action: () => router.push('/changelog')
        },
        {
            label: theme === 'dark' ? 'Light Mode' : 'Dark Mode',
            icon: theme === 'dark' ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />,
            action: (e: any) => toggleTheme(e)
        },
        {
            label: 'Say Hello',
